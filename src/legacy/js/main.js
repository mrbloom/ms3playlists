...
function get_series_name(prefix,i){
    ...
    function inside(i,name,prefix){
        let [min,max] = get_min_max(name)
        return (min <= i && i<= max) ? prefix : undefined
    }

    prefixies_funcs = {
        "Помста1": i => inside(i,"Помста1","p007289-01-"),
        "Помста2": i => inside(i,"Помста2","p008255-02-"),
        "Помста3": i => inside(i,"Помста3","p013532-03-"),
        "Помста4": i => inside(i,"Помста4","p014107-04-"),
        "Рятівники": spasateli,
        "Віталька": vitalka,
...
function get_min_max(series_name){
    switch(series_name){
        case "Помста1" : return [1,21];
        case "Помста2" : return [1,20];
        case "Помста3" : return [1,50];
        case "Помста4" : return [1,26];
